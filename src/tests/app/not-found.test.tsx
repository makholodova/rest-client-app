...
jest.mock('@/components/layout/page/page', () => ({
    __esModule: true,
    default: ({ children, centered }: { children: React.ReactNode; centered?: boolean }) => (
        <div data-testid="page" data-centered={centered}>{children}</div>
    ),
}));
...
jest.mock('@/components/ui/button-link/button-link', () => ({
    ...
    default: ({ href, children }: { href: string; children: React.ReactNode }) => (
        <a data-testid="btn" href={href}>{children}</a>
    ),
}));
